...
function handleSubmit({ navigation, fertilityQuestions, dispatch, }) {
    ...
    if (fertilityQuestions.ivfconceived === "yes") {
        dispatch(actions.change('fertilityQuestions.questionTot', (fertilityQuestions.questionTot+4)));
        navigation.navigate("SingletonMultiBirth")
    } else {
        navigation.navigate("Report")
    }
}

const IVFConceived = ({ navigation, fertilityQuestions, dispatch }) => (
    <Content >
...
export default connect(({ nav, fertilityQuestions }) => ({ nav, fertilityQuestions }))(IVFConceived); 