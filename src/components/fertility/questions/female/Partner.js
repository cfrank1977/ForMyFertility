...
function handleSubmit({ navigation, fertilityQuestions, dispatch, }) {
    ...
    if (fertilityQuestions.partner === "yes") {
        dispatch(actions.change('fertilityQuestions.questionTot', ++fertilityQuestions.questionTot));
        navigation.navigate("PartnerIssues")
    } else {
        navigation.navigate("CurrentIVF")
    }
}

const Partner = ({ navigation, fertilityQuestions, dispatch }) => (
    <Content >
...
export default connect(({ nav, fertilityQuestions }) => ({ nav, fertilityQuestions }))(Partner); 